...
class Person(me.Document):#inheritance Person inherited from Document
   
    name = me.StringField(required=True, max_length=100)
    gender = me.StringField(required=True, choices=['Male', 'Female', 'Other'])#creating a person's model 
    ...
    print(f"\nConnected to MongoDB using {Dbname} !!!") #should give two more fields password and useraname if we change the settings n the mongodb
...
def add_person():
    ...
    person.save() #Connects to mongodb and saves the changes and again disconnects the mongodb
